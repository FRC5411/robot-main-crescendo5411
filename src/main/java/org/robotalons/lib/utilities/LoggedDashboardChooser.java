...
public class LoggedDashboardChooser<V> implements LoggedDashboardInput {
  ...
  public LoggedDashboardChooser(String key, SendableChooser<V> chooser) {
    ...
    try {
      Field mapField = SendableChooser.class.getDeclaredField("m_listener");
      mapField.setAccessible(true);
      callback = (Consumer<V>) mapField.get(chooser);
    } catch (NoSuchFieldException
        | SecurityException
        | IllegalArgumentException
        | IllegalAccessException e) {
  ...
  public void periodic() {
    if (!Logger.hasReplaySource()) {
      if(selectedValue != sendableChooser.getSelected()) {
        selectedValue = sendableChooser.getSelected();
        if(callback != (null)) {
          callback.accept(options.get(selectedValue));
        }
      }
    }
