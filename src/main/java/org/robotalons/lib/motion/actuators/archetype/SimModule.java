...
public class SimModule<Controller extends DCMotorSim> extends Module {
  ...
  public SimModule(final ModuleConfiguration<Controller> Constants) {
    ...
    RotationalAbsoluteOffset = new Rotation2d(Math.random() * 2d * Math.PI);
    TRANSLATIONAL_CONTROLLER.setState((Math.random() * 2d * Math.PI), TRANSLATIONAL_CONTROLLER.getAngularVelocityRadPerSec());
  ...
  public synchronized void update() {
    ...
    synchronized(STATUS) {
      STATUS.TranslationalPositionRadians = TRANSLATIONAL_CONTROLLER.getAngularPositionRad() *  MODULE_CONSTANTS.TRANSLATIONAL_GEAR_RATIO;
      STATUS.TranslationalVelocityRadiansSecond = TRANSLATIONAL_CONTROLLER.getAngularVelocityRadPerSec() *  MODULE_CONSTANTS.TRANSLATIONAL_GEAR_RATIO;
